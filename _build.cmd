...
echo Assembling...
rgbds\rgbasm -v -p 255 -o %filename%.o %source%.asm
if %ERRORLEVEL% neq 0 goto assemble_fail

echo Linking...
rgbds\rgblink -v -p 255 -m %filename%.map -n %filename%.sym -d -o %filename%.gb %filename%.o
...
if %nofix% equ 0 (
	...
	rgbds\rgbfix -v -p 255 %filename%.gb
)
